...
def create_stream():
  global stream_handle, settings, loop_running, hmqtt, log
  ...
  if settings.do_rtsp:
    ...
    #cc = f'appsrc ! videoconvert ! x264enc noise-reduction=10000 speed-preset=ultrafast tune=zerolatency ! rtph264pay config-interval=1 pt=96 ! udpsink host=127.0.0.1 port=5000 sync=false'
    cc = f'appsrc ! queue ! videoconvert ! video/x-raw ! omxh264enc ! video/x-h264 ! h264parse ! rtph264pay config-interval=1 pt=96 ! gdppay ! udpsink host=127.0.0.1 port=5000'
    stream_handle = cv2.VideoWriter(cc,cv2.CAP_GSTREAMER, 15, (640,480));
    '''
    # write to a file (debugging?) 
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    stream_handle = cv2.VideoWriter('/tmp/tracker.avi',fourcc, 15, (640,480)); 
    ''' 
    if not stream_handle.isOpened :
      log.warn('failed to open rtp stream') 
